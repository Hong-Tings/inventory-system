...
function addItem() {
  form.value.items.push({ productId: null, quantity: 1, unitPrice: null, amount: null, batchNo: '' })
}
...
function onProductChange(item) {
  ...
  if (p && !item.unitPrice) item.unitPrice = p.salePrice
}
...
<template>
  <view class="page">
    <view class="section">
      ...
      <view class="form-row">
        <view class="form-item flex-1">
          ...
          <input v-model="form.salesman" class="input" placeholder="选填" />
        </view>
        <view class="form-item flex-1">
          <text class="label">外部单号</text>
          <input v-model="form.externalOrderNo" class="input" placeholder="选填" />
        </view>
      </view>
    </view>
    ...
    <button class="submit-btn" :loading="submitting" @click="handleSubmit">确认出库</button>
  </view>
</template>
