...
第12次修改
1.商品列表：我的火焰豌豆总共99个，主仓库88个，二号仓库11个，但是在筛选进行仓库筛选后，不管是主仓还是二号仓库存还是99  已修复
2.商品列表的库存状态筛选添加正常状态，  已修复
3.有时候新增商品点击保存显示系统内部错误: [DuplicateKeyException] ### Error updating database. Cause: java.sql.SQLIntegrityConstraintViolationException: Duplicate entry '51' for key 'product.PRIMARY' ### The error may exist in com/inventory/product/mapper/ProductMapper.java (best guess) ### The error may involve com.inventory.product.mapper.ProductMapper.insert-Inline ### The error occurred while setting parameters ### SQL: INSERT INTO product ( id, code, name, category_id, spec, unit, purchase_price, sale_price, image_url, min_stock, max_stock, status, remark, deleted, create_time, update_time ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) ### Cause: java.sql.SQLIntegrityConstraintViolationException: Duplicate entry '51' for key 'product.PRIMARY' ; Duplicate entry '51' for key 'product.PRIMARY'，
已修复

4.库存预警显示是哪个仓库的商品，因为两个仓库如果有同一个商品的情况下，库存预警不能反映准确的信息  
5.导出的excel日期宽度过小，导致显示######   -->
<!-- 6.导出客户excel显示{"code":500,"message":"系统内部错误: [HttpRequestMethodNotSupportedException] Request method 'GET' is not supported","data":null,"timestamp":1777401786676} 
7.导出仓库管理excel显示{"code":500,"message":"系统内部错误: [HttpRequestMethodNotSupportedException] Request method 'GET' is not supported","data":null,"timestamp":1777402087333} -->

第13次修改
1.提交盘点单报系统内部错误: Unknown column 'location_id' in 'field list'，原因是 StockTake.java 实体中 locationId 字段在数据库 stock_take 表中不存在，MyBatis-Plus 自动映射时报错，添加 @TableField(exist = false) 修复
2.盘点调整 adjust() 方法查询库存时缺少 warehouseId 过滤条件，导致跨仓库盘点时会错误清零其他仓库的同商品库存，已添加仓库条件修复
3.盘点手动添加项 addItem() 查询账面数量时同样缺少 warehouseId 过滤条件，导致跨仓库汇总账面数量，已添加仓库条件修复
4.采购入库、销售出库、库存调拨的 submit/cancel 方法未加并发锁，多人同时操作同一商品可能导致库存超卖或回滚错乱，已添加 synchronized 关键字保护

第14次修改
1.库存金额计算方式改为移动加权平均法。原方案使用商品资料中的采购参考价×库存数量，当进价波动时金额不准确。改为在 inventory 表新增 cost_price 字段，每次采购入库时按公式（原库存总金额+本次进货金额）÷（原库存总数量+本次进货数量）重新计算加权平均成本价，仓库列表金额改为 cost_price×数量。调拨入库时沿用调出仓库的成本价并重新计算目标仓库的加权平均。已更新数据库表结构、WarehouseService、PurchaseOrderService、TransferService
<!-- 8.导出的采购入库和销售出库excel把已作废的数据也导出来了
9.库存列表导出的excel还有库位栏，库位是已经删除的 -->
