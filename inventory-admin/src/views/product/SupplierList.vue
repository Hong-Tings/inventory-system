...
const form = reactive<Partial<Supplier>>({ name: '', code: '', contact: '', phone: '', address: '', remark: '' })

async function fetchData() {
  loading.value = true
  try {
    const res = await request.get('/supplier/page', { params: query })
...
function openEdit(row: Supplier) { Object.assign(form, row); dialogVisible.value = true }
async function handleSave() {
  if (!form.name) return
  if ((form as any).id) await request.put(`/supplier/${(form as any).id}`, form)
  else await request.post('/supplier', form)
...
<template>
  <div>
    <div class="page-header"><h2>供应商管理</h2><div><el-button @click="handleExport">导出Excel</el-button><el-button type="primary" @click="openCreate">新增供应商</el-button></div></div>
    <div class="search-bar">
      <el-input v-model="query.name" placeholder="供应商名称" clearable style="width:200px" @keyup.enter="handleSearch" @clear="handleSearch" />
